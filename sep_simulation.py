...
def proposal(dmin, No):
    ...
    p = (244/256)*(1 - gamma.cdf(R**2,1,0,No)) + (12/256)*(1 - (gamma.cdf(R**2,1,0,No)/2 + (1/2)*(1-2*(0.5 - math.erf(dmin/(2*sigma)/math.sqrt(2))/2))))#cdf(x, a, loc=0, scale=1)
    return p
...
for i in noise:
    ...
    temp2 = np.array([existing(2, i, 711/128, 171/32)]) #[18]
...
noise  = 10*np.log10( (141.01/8) / noise) #Eb/No
plt.plot(noise, y1, 's')
plt.gca().set_yscale('log')
plt.plot(noise, y2, 'h')
plt.gca().set_yscale('log')
plt.plot(noise, y4, '--')
plt.gca().set_yscale('log')
plt.legend(["proposed_fix", "existing", "proposed_first"])
plt.ylabel('SEP of irregular 256-TQAM')
plt.xlabel("Eb/No")
plt.show()
